...
# 1. Log in to the PubDic and get the authentication token
res = RestClient.post(
...
resource = RestClient::Resource.new( 
	# "localhost:3000/dictionaries.json",
	# rest_api.to_s,
	"#{rest_api.to_s}?auth_token=#{auth_token}",
	:timeout => 300, 
	:open_timeout => 300 )
res = resource.post( 
...
res = RestClient.delete(
		"localhost:3000/users/sign_out.json",
		:params 		=> {:auth_token => auth_token},
		:content_type 	=> :json,
		:accept			=> :json,
	  )
$stderr.puts "Response code: #{res.code}"
$stderr.puts JSON.parse(res)
$stderr.puts
