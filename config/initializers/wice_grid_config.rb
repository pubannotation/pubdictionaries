...
# This code is to avoid errors caused by wice_grid in Rails 6.1.
# Without this code, You would get the error
# "FrozenError (can't modify frozen ActiveRecord::ConnectionAdapters::PostgreSQL::Column: ~"
# when displaying a table using wice_grid.
...
module Wice
  class WiceGrid
    def declare_column(
      ...
      if column
        conditions_generator = ActiveRecordColumnWrapper.new(column, @status[:f], main_table, table_alias, custom_filter_active, filter_type, @table_column_matrix.default_model_class)
  ...
  class TableColumnMatrix
    def init_columns_of_table(model)
  ...
  class ActiveRecordColumnWrapper
    def initialize(column, all_filter_params, main_table, table_alias, custom_filter_active, filter_type, default_model_class)
      @column = column
    ...
    def alias_or_table_name(table_alias)
      table_alias || @default_model_class.table_name
    end
  end
end
